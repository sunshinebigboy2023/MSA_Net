...
class MaskedBCEWithLogitsLoss(nn.Module):
    ...
    def __init__(self, ignore_zero_targets=True):
        super(MaskedBCEWithLogitsLoss, self).__init__()
        self.ignore_zero_targets = ignore_zero_targets
        self.loss = nn.BCEWithLogitsLoss(reduction='sum')

    def forward(self, pred, target, umask):
        """
        pred -> [batch*seq_len] or [batch*seq_len, 1]
        target -> [batch*seq_len] or [batch, seq_len]
        umask -> [batch, seq_len]
        """
        mask = umask.view(-1).float()
        pred = pred.view(-1)
        target = target.view(-1).float()

        valid = mask > 0
        if self.ignore_zero_targets:
            valid = valid & (target != 0)

        if valid.sum() <= 0:
            return pred.new_zeros(())

        binary_target = (target[valid] > 0).float()
        loss = self.loss(pred[valid], binary_target) / valid.sum().float()
        return loss
