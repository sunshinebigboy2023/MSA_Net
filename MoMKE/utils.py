...
def generate_mask_tensor(seqlen, batch, test_condition, first_stage, device=None, dtype=torch.float32):
    if first_stage:
        values = [1.0, 1.0, 1.0]
    else:
        values = [
            1.0 if 'a' in test_condition else 0.0,
            1.0 if 't' in test_condition else 0.0,
            1.0 if 'v' in test_condition else 0.0,
        ]
    base = torch.tensor(values, device=device, dtype=dtype).view(1, 1, 3)
    return base.expand(seqlen, batch, 3)
...
def generate_inputs(audio_host, text_host, visual_host, audio_guest, text_guest, visual_guest, qmask):
    ...
    tmask = tmask.unsqueeze(2).expand_as(feat1) # -> [seqlen, batch, featdim]
