...
class CMUMOSIDataset(Dataset):
    ...
    def __init__(self, label_path, audio_root, text_root, video_root):
        ...
        for ii, vid in enumerate(sorted(self.videoIDs)):
            ...
            self.videoAudioHost[vid] = np.asarray(self.videoAudioHost[vid], dtype=np.float32)
            self.videoTextHost[vid] = np.asarray(self.videoTextHost[vid], dtype=np.float32)
            self.videoVisualHost[vid] = np.asarray(self.videoVisualHost[vid], dtype=np.float32)
            self.videoAudioGuest[vid] = np.asarray(self.videoAudioGuest[vid], dtype=np.float32)
            self.videoTextGuest[vid] = np.asarray(self.videoTextGuest[vid], dtype=np.float32)
            self.videoVisualGuest[vid] = np.asarray(self.videoVisualGuest[vid], dtype=np.float32)
            self.videoLabelsNew[vid] = np.asarray(self.videoLabelsNew[vid], dtype=np.float32)
            self.videoSpeakersNew[vid] = np.asarray(self.videoSpeakersNew[vid], dtype=np.float32)


    ## return host(A, T, V) and guest(A, T, V)
    def __getitem__(self, index):
        vid = self.vids[index]
        return torch.from_numpy(self.videoAudioHost[vid]),\
               torch.from_numpy(self.videoTextHost[vid]),\
               torch.from_numpy(self.videoVisualHost[vid]),\
               torch.from_numpy(self.videoAudioGuest[vid]),\
               torch.from_numpy(self.videoTextGuest[vid]),\
               torch.from_numpy(self.videoVisualGuest[vid]),\
               torch.from_numpy(self.videoSpeakersNew[vid]),\
               torch.ones(len(self.videoLabelsNew[vid]), dtype=torch.float32),\
               torch.from_numpy(self.videoLabelsNew[vid]),\
               vid
    ...
    def collate_fn(self, data):
        ...
        for i, items in enumerate(columns):
            if i <= 5:
                datnew.append(pad_sequence(list(items)))
            elif i <= 8:
                datnew.append(pad_sequence(list(items), True))
            else:
                datnew.append(list(items))
        return datnew
